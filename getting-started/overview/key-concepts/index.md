...
- [Coworkers Space](./coworkers-space) — How multiple client applications are grouped so they can share data through the Platform without calling each other directly. Covers client registration, JWT claims, document-level sharing via `clients[]`, and CQRS webhook delivery.
